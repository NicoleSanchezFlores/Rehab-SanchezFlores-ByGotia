---
title: "Determining the Association Between Drug Rehabilitation Patient Characteristics and Primary Substance Choice"
format: pdf
...
Given the non-ordinal and multi-category nature of the primary substance outcome, a multinomial logistic regression model was therefore selected for our analysis @Bilder2015. Binary logistic regression was not chosen in this instance as it would cause all substances to be collapsed into a single category which would prevent a deeper understanding of the substance-specific relationships between the explanatory variables.
...
To conduct multinomial logistic regression the `multinom()` function was used from the `nnet` package @newtest.
...
final_table <- 
  ...
  left_join(
    as.data.frame(s$standard.errors) |>
      rownames_to_column("Outcome") |>
      pivot_longer(
        -Outcome,
        names_to = "Predictor",
...
mutate(
  Predictor = dplyr::recode(
    Predictor,
    "(Intercept)"        = "Intercept",
    "gender_labelFemale" = "Gender: Female",
    "age"                = "Age",
    "home_labelYes"      = "Homeless: Yes",
    "ces_d"              = "Depression Score (CES-D)"
  )
)
...
kable(
  final_table,
  digits = 3,
  caption = "Multinomial Logistic Regression Results: Coefficients and Standard Errors"
)
...
table_1 <- table_1 |>
  mutate(
    Characteristic = case_when(
      Characteristic == "Sample"        ~ "Sample Size (N)",
      Characteristic == "Alcohol"       ~ "Primary Substance: Alcohol",
      Characteristic == "Cocaine"       ~ "Primary Substance: Cocaine",
      Characteristic == "Heroin"        ~ "Primary Substance: Heroin",
      Characteristic == "Age_Mean"      ~ "Age, Mean",
      Characteristic == "Age_SD"        ~ "Age, SD",
      Characteristic == "Female_perc"   ~ "Female (%)",
      Characteristic == "Homeless_perc" ~ "Homeless (%)",
      Characteristic == "CESD_Mean"     ~ "CES-D Score, Mean",
      Characteristic == "CESD_SD"       ~ "CES-D Score, SD",
      TRUE ~ Characteristic
    )
  )
...
kable(
  table_1,
  digits = 2,
  caption = "Demographic Characteristics of Patient Sample (N = 469)",
...
Anova(mod.fit) |>
  ...
  mutate(
    term = case_when(
      term == "gender_label" ~ "Gender",
      term == "age"          ~ "Age",
      term == "home_label"   ~ "Homeless Status",
      term == "ces_d"        ~ "Depression Score (CES-D)",
      TRUE ~ term
    )
  ) |>
  kable(
    digits = 3,
    caption = "ANOVA Table for Multinomial Logistic Regression Model",
...
rrr_model <- rrr_model |>
  mutate(
    term = case_when(
      ...
      term == "gender_labelFemale" ~ "Female",
      term == "age" ~ "Age",
      term == "home_labelYes" ~ "Homeless,Yes",
...
While relative risk ratios quantify marginal associations, predicted probabilities were calculated to illustrate how these associations translate into absolute likelihoods of primary substance choice across the observed covariate values @Bilder2015.
...
kable(
  pi_table,
  digits = 3,
  caption = "Summary of the Predicted Probabilites by Substance"
) |>
  kable_styling(full_width = FALSE)
...
ggplot(pp_long_age,
  ...
  labs(
    title = "Figure 1. Predicted Probability of Primary Substance by Age",
    x = "Age",
...
##  Depression Score and Primary Substance
...
ggplot(pp_long,
  ...
  geom_line(linewidth = 1) +
  labs(
    title = "Figure 2. Predicted Probability of Primary Substance\nby CES-D Score",
    x = "CES-D Score",
    y = "Predicted Probability"
  ) +
  theme_minimal() +
...
# Limitations
Several limitations should be considered when interpreting the findings of this study. These limitations may affect the generalizability of the results and how they apply to current substance use patterns.
